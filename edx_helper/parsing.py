    ...
    def extract_courses(self, response):
        ...
        for item in response["courses"]:
            ...
            if item["courseRun"]["isStarted"]:
                ...
                item["enrollment"]["isAudit"]
                and not item["enrollment"]["isAuditAccessExpired"]
            ):
                course_state = "Started"
            else:
...
class ClassicEdXPageExtractor(PageExtractor):
    ...
    def extract_units_from_html(self, page, BASE_URL, file_formats):
        ...
        # you don't need to !
        re_units = re.compile(
            '(<div?[^>]id="seq_contents_\d+".*?>.*?<\/div>)', re.DOTALL
        )
        units = []
    ...
    def extract_unit(self, text, BASE_URL, file_formats):
        ...
        available_subs_url, sub_template_url = self.extract_subtitle_urls(
            text, BASE_URL
        )
    ...
    def extract_video_youtube_url(self, text):
        re_video_youtube_url = re.compile(
            r"data-streams=&#34;.*?1.0\d+\:(?:.*?)(.{11})"
        )
        ...
        if match_video_youtube_url is None:
            re_video_youtube_url = re.compile(
                r"https://www.youtube.com/embed/(.{11})\?rel="
            )
            match_video_youtube_url = re_video_youtube_url.search(text)
    ...
    def extract_subtitle_urls(self, text, BASE_URL):
        re_sub_template_url = re.compile(
            r'data-transcript-translation-url=(?:&#34;|")([^"&]*)(?:&#34;|")'
        )
        re_available_subs_url = re.compile(
            r'data-transcript-available-translations-url=(?:&#34;|")([^"&]*)(?:&#34;|")'
        )
    ...
    def extract_sections_from_html(self, page, BASE_URL):
        ...
        sections = [section for section in sections if section.name and section.url]
    ...
    def extract_courses_from_html(self, page, BASE_URL):
        ...
        for course_soup in courses_soup:
            ...
            try:
                ...
                if (
                    course_url.endswith("info")
                    or course_url.endswith("info/")
                    or course_url.endswith("course")
                    or course_url.endswith("course/")
                ):
                    course_state = "Started"
...
class CurrentEdXPageExtractor(ClassicEdXPageExtractor):
    ...
    def extract_unit(self, text, BASE_URL, file_formats):
        ...
        for match_metadata in match_metadatas:
            ...
            available_subs_url = (
                BASE_URL + metadata["transcriptAvailableTranslationsUrl"]
            )
            sub_template_url = BASE_URL + metadata["transcriptTranslationUrl"].replace(
                "__lang__", "%s"
            )
    ...
    def extract_sections_from_html(self, page, BASE_URL):
        ...
        def _make_subsections(section_soup):
            try:
                subsections_soup = section_soup.find_all(
                    "div", attrs={"class": "menu-item"}
                )
            except AttributeError:
                return []
            # FIXME correct extraction of subsection.name (unicode)
            subsections = [
                SubSection(position=i, url=BASE_URL + s.a["href"], name=s.p.string)
                for i, s in enumerate(subsections_soup, 1)
            ]
        ...
        soup = BeautifulSoup(page)
        sections_soup = soup.find_all(
            "div", attrs={"class": "chapter-content-container"}
        )
...
class NewEdXPageExtractor(CurrentEdXPageExtractor):
    ...
    def extract_sections_from_html(self, page, BASE_URL):
        ...
        def _make_subsections(section_soup):
            ...
            # FIXME correct extraction of subsection.name (unicode)
            subsections = [
                SubSection(position=i, url=s.a["href"], name=s.a.h4.string.strip())
                for i, s in enumerate(subsections_soup, 1)
            ]
    ...
    def extract_unit(self, unit_soup, BASE_URL, file_formats):
        ...
        for xblock in xblock_list:
            ...
            if xblock_type == "video":
                ...
                sub_template_url = None
                mp4_urls = []
                origin_available_subs_url = None
                video_data_metadata = []
                video_download_button = xblock.find(
                    "a", "btn-link video-sources video-download-button"
                )
                if video_download_button:
                    mp4_urls.append(video_download_button["href"])
                subtitle_download_button = xblock.find(
                    "a", text=re.compile("^(下载|Download) SubRip")
                )
                if subtitle_download_button:
                    origin_available_subs_url = BASE_URL + subtitle_download_button["href"]

                re_metadata = re.compile(r"data-metadata=.*ytTestTimeout.: \d+}")
                if re_metadata.search(str(unit_soup)):
                    match_metadatas = re_metadata.findall(str(unit_soup))
                    for match_metadata in match_metadatas:
                        match_metadata = html.unescape(match_metadata)
                        match_metadata = re.sub(
                            "data-metadata='", '"data-metadata":', match_metadata
                        )
                        metadata = json.loads("{" + match_metadata + "}")
                        metadata = metadata["data-metadata"]
                        video_data_metadata.append(metadata)
                        available_subs_url = (
                            BASE_URL + metadata["transcriptAvailableTranslationsUrl"]
                        )
                        sub_template_url = BASE_URL + metadata[
                            "transcriptTranslationUrl"
                        ].replace("__lang__", "%s")

                if False and not video_download_button and not subtitle_download_button:
                    re_metadata = re.compile(r"data-metadata=&#39;(.*?)&#39;")
                    match_metadatas = re_metadata.findall(str(unit_soup))
                    for match_metadata in match_metadatas:
                        metadata = html.unescape(match_metadata)
                        metadata = json.loads(html.unescape(metadata))
                        re_video_speed = re.compile(r"1.0\d+\:(?:.*?)(.{11})")
                        match_video_youtube_url = re_video_speed.search(
                            metadata["streams"]
                        )
                        if match_video_youtube_url is not None:
                            video_id = match_video_youtube_url.group(1)
                        ...
                        mp4_urls = [
                            url for url in metadata["sources"] if url.endswith(".mp4")
                        ]
                videos.append(
                    Video(
                        video_youtube_url=video_youtube_url,
                        ...
                        sub_template_url=sub_template_url,
                        mp4_urls=mp4_urls,
                        video_data_metadata=video_data_metadata,
                        origin_available_subs_url=origin_available_subs_url,
                    )
                )
            else:
...
def get_page_extractor(url):
    ...
    if url.startswith("https://courses.edx.org") or url.startswith(
        ...
        url.startswith("https://edge.edx.org")
        or url.startswith("https://lagunita.stanford.edu")
        or url.startswith("https://www.fun-mooc.fr")
    ):
        return NewEdXPageExtractor()
    else:
